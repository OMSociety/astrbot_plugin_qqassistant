...
class JoinHandle:
    ...
    async def event_monitoring(self, event: AiocqhttpMessageEvent):
        ...
        # 群公告发布
            ...
            raw.get("post_type") == "notice"
            and raw.get("notice_type") == "group_notice"
        ):
            if await self.db.get(gid, "notice_notify", True):
                sender_name = await get_nickname(event, uid)
                ...
                if isinstance(msg_obj, dict):
                    ...
                    if notice_text:
                        notice_lines.append(notice_text.strip())
                    if images:
                        notice_lines.append(f"（附图 {len(images)} 张）")
                    await event.send(event.plain_result("\n".join(notice_lines)))
