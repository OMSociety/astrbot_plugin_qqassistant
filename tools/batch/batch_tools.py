...
class BatchToolsHandle:
    ...
    async def batch_ban(
        self, event: AiocqhttpMessageEvent, user_ids: list[str], duration: int
    ) -> dict[str, Any]:
        """
        批量禁言用户
        :param event: 消息事件
        ...
        for user_id in user_ids:
            try:
                await event.bot.set_group_ban(
                    group_id=event.group_id, user_id=user_id, duration=duration
                )
                success_count += 1
    ...
    async def batch_set_card(
        self, event: AiocqhttpMessageEvent, user_cards: dict[str, str]
    ) -> dict[str, Any]:
        """
        批量修改群名片
        :param event: 消息事件
        ...
        for user_id, new_card in user_cards.items():
            try:
                await event.bot.set_group_card(
                    group_id=event.group_id, user_id=user_id, card=new_card
                )
                success_count += 1
            except Exception as e:
                fail_count += 1
    ...
    async def batch_send_msg(
        self, event: AiocqhttpMessageEvent, group_ids: list[str], message: str
    ) -> dict[str, Any]:
        """
        批量发送群消息
        :param event: 消息事件
